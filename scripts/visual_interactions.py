...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='friendbook-interactions-') as folder:
        ...
        for dark in (False, True):
            ...
            window.editor.label_picker.toggle.setChecked(True)
            window.editor.tabs.widget(0).ensureWidgetVisible(window.editor.label_picker)
            capture('labels-' + suffix)
            window.discard_editor()
            if not dark:
                window.navigate('统计')
                capture('stats-labels-bar', 800)
                window.stats_charts['标签'].choice.setCurrentIndex(1)
                capture('stats-labels-donut', 800)
            window.navigate('概览')
            capture('home-' + suffix)
